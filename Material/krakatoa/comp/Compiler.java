...
public class Compiler {
	...
	private KraClass classDec() {
		...
		while (lexer.token == Symbol.PRIVATE || lexer.token == Symbol.PUBLIC) {
			...
			String name = lexer.getStringValue(); // --------- AQUI A GENTE
													// PRECISA ADICIONAR AO
													// LOCAL DA VARIAVEL ESSAS
													// COISAS ---------
			lexer.nextToken();
			...
			if (lexer.token == Symbol.LEFTPAR) {
				...
				if (qualifier == Symbol.PRIVATE) {
					Method privateMethod = methodDec(t, name, qualifier);
					listaPrivateMetodos.addMethod(privateMethod);
					classAtual.setPrivateMethod(privateMethod);
				} else {
					Method publicMethod = methodDec(t, name, qualifier);
					listaPublicMetodos.addMethod(publicMethod);
					classAtual.setPublicMethod(publicMethod);
				}
	...
	private InstanceVariableList instanceVarDec(Type type, String name) {
		...
		classAtual.setInstanceVariable(new InstanceVariable(name, type));

		while (lexer.token == Symbol.COMMA) { // --------- AQUI VERIFICA SE
												// POSSUI MAIS VARIÁVEIS
												// ---------
			lexer.nextToken();
			...
			classAtual.setInstanceVariable(new InstanceVariable(variableName, type));
		}
	...
	private StatementList statementList() {
		...
		while ((tk = lexer.token) != Symbol.RIGHTCURBRACKET && tk != Symbol.ELSE) { // Aqui
																					// a
																					// gente
																					// vai
																					// salvando
	...
	private Statement statement() {
		...
		default:
			if (lexer.token == Symbol.LITERALINT || lexer.token == Symbol.LITERALSTRING || lexer.token == Symbol.FALSE
		...
		if (lexer.token == Symbol.INT || lexer.token == Symbol.BOOLEAN || lexer.token == Symbol.STRING) {
			...
			if (lexer.token == Symbol.ASSIGN) {
				...
				if (left.getType() != right.getType()) {
					...
					if (left.getType() instanceof KraClass && (right.getType() == Type.intType
							|| right.getType() == Type.booleanType || right.getType() == Type.stringType)) {
						signalError.showError(
								"Type error: the type of the expression of the right-hand side is a basic type and the type of the variable of the left-hand side is a class");
					}

					if ((left.getType() == Type.intType || left.getType() == Type.booleanType
							|| left.getType() == Type.stringType) && right.getType() instanceof KraClass) {
						signalError.showError(
								"Type error: type of the left-hand side of the assignment is a basic type and the type of the right-hand side is a class");
					}

					if ((left.getType() == Type.intType || left.getType() == Type.booleanType
							|| left.getType() == Type.stringType) && right.getType() == Type.voidType) {
				...
				if (left instanceof VariableExpr) {
					...
					left = null;
				} else if (left instanceof MessageSendToVariable || left instanceof MessageSendToSuper
	...
	private ReturnStatement returnStatement() {
		...
		if (!metodoAtual.getType().getName().equals(rtExpr.getType().getName()))
				...
				if (lexer.token != Symbol.IDENT) {
					...
					if (lexer.token == Symbol.DOT) {
						...
						if (calledMethod == null) {
							signalError.showError(
									"Method '" + id + "' was not found in class '" + firstId + "' or its superclasses");
						}
						exprList = this.realParameters();

						return new MessageSendToVariable(symbolTable.getInLocal(firstId), calledMethod, exprList);
					} else {
						KraClass classedoIdent = symbolTable
								.getInGlobal(symbolTable.getInLocal(firstId).getType().getName());

						return new MessageSendToInstance(symbolTable.getInLocal(firstId),
								classedoIdent.getVariable(id));
					}
				}
			}
			break;
		case THIS:
			...
			if (lexer.token != Symbol.DOT) {
				...
				if (lexer.token == Symbol.LEFTPAR) {
					...
					KraClass calledClass = symbolTable.getInGlobal(id);
					if (!classAtual.hasInstanceVariable(id) || id == null) {
					...
					if (lexer.token == Symbol.LEFTPAR) {
						Method mt = calledClass.getMethod(id);
						exprList = this.realParameters();
						
						return new MessageSendToSelf(classAtual, mt, exprList);
					} else {
						InstanceVariable v = calledClass.getVariable(id);
						
						return new MessageSendToSelf(classAtual, v);
					}
				} else {
					InstanceVariable v = classAtual.getVariable(id);
